...
//操作结果：将元素e追加到栈顶，如成功则返回SUCCESS，否则如动态内存已耗尽
//将返回OVER_Flow
...
//操作结果：如果栈非空，删除栈顶元素，并用e返回栈顶元素，返回SUCCESS，否则
//返回UNDER_FLOW
    if (Empty()) {
        //栈空
        return UNDER_FLOW;
...
//操作结果：如果栈非空，用e返回栈顶元素，返回SUCCESS，否则
//返回UNDER_FLOW
    if (Empty()) {
        //栈空
        return UNDER_FLOW;
...
//操作结果：由copy构造新栈——复制构造函数模板
    if (copy.Empty()) {
        ...
        //copy非空，复制栈
        top = new Node<ElemType>(copy.top -> data);           //生成当前栈栈顶
        Node<ElemType> *buttomPtr = top;                      //生成当前栈栈底
