...
{
    length = (int)strlen(copy.CStr());				// 串长
...
{
    length = (int)strlen(copy);						// 串长
    strVal = new char[length + 1];				// 分配存储空间
    strcpy(strVal, copy);						// 复制串值
}

String::String(SimpleLinkList<char> &copy)
...
// 操作结果：从输入流读入串
{
    SimpleLinkList<char> temp;				    // 临时线性表
    int size = 0;								// 初始线性表长度
    char c;										// 临时字符
...
int SimpleIdex(const String &first, const String &second, int pos = 0)
{
    int i = pos, j = 0;
